...
from lxmert.utils import WEIGHTS_NAME, Config, cached_path, hf_bucket_url, is_remote_url, load_checkpoint
...
def norm_box(boxes, raw_sizes):
    ...
    normalized_boxes[:, :, (0, 2)] /= torch.unsqueeze(torch.unsqueeze(raw_sizes[:, 1], -1), -1)#raw_sizes[:, 1]
    normalized_boxes[:, :, (1, 3)] /= torch.unsqueeze(torch.unsqueeze(raw_sizes[:, 0], -1), -1)#raw_sizes[:, 0]
...
class GeneralizedRCNN(nn.Module):
    ...
    def inference(
        ...
        **kwargs,
        ...
        normalized_boxes = norm_box(boxes.to(self.device), original_sizes)
        return OrderedDict(
            {
                "obj_ids": classes,
